Template.mobileusers.rendered = function() {
  ...
  Template.mobileusers.helpers({
    ...
    displayRewards: function(user) {
      return Template.mobileusers.formatRewards(user)
    }
  });
  
  Template.mobileusers.formatRewards = function(user) {
    var rewards = []
    if (user.reward_sbd_balance.split(' ')[0] > 0)
