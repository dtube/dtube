// https://github.com/fnando/sparkline
  ...
  window.sparkline = function(svg, entries, options) {
    removeChildren(svg);
    ...
    const zeroWidth = options.zeroWidth || 2;
    ...
    const fullWidth = parseFloat(svg.attributes.width.value);
    ...
    const pathY = getY(min, max, height, strokeWidth + spotRadius, values[0]);
    let pathCoords = `M${spotDiameter} ${pathY}`;
  
    values.forEach((value, index) => {
      const x = index * offset + spotDiameter;
      const y = getY(min, max, height, strokeWidth + spotRadius, value);
    ...
    const zero = buildElement("line", {
        class: "sparkline--zero",
        x1: 0,
        x2: fullWidth,
        y1: getY(min, max, height, strokeWidth + spotRadius, values[0]),
        y2: getY(min, max, height, strokeWidth + spotRadius, values[0]),
        "stroke-width": zeroWidth
    });
    ...
    interactionLayer.addEventListener("mouseout", event => {
      cursor.setAttribute("y1", offscreen);
      cursor.setAttribute("y2", offscreen);
